...
//	USART_Enable(USARTx, ENABLE);			//使能USART	
//	/*配置中断*/
...
//	DMA_InitStructure.PeriphAddr = (uint32_t)&(USARTx->DAT);				//外设地址
//	DMA_InitStructure.MemAddr    = (uint32_t)usartx_rx_buf;					//本例程发送和接收共用一个数组，接收到数据之后立即返回
//	DMA_InitStructure.Direction  = DMA_DIR_PERIPH_DST;							//外设作为目标设备
//	DMA_InitStructure.BufSize    = USARTx_MAX_UART_BUF;							//发送数据大小，每次发送可以重新定义
...
//	DMA_InitStructure.BufSize    = USARTx_MAX_UART_BUF;					//默认接收数据大小
...
//{
//	DMA_EnableChannel(USARTx_Tx_DMA_Channel, DISABLE);	  //关闭DMA通道	 
//	DMA_SetCurrDataCounter(USARTx_Tx_DMA_Channel,Length); //设置传输长度
//	DMA_EnableChannel(USARTx_Tx_DMA_Channel, ENABLE);     //使能DMA通道
//}
